 ...
 */
namespace Lof\StoreLocator\Block\Adminhtml\StoreLocator\Edit\Tab;
...
class Tags extends \Magento\Backend\Block\Widget\Grid\Extended
{
     ...
     * @param \Lof\StoreLocator\Model\Category
     * @param array
     */
    public function __construct(
        ...
        \Lof\StoreLocator\Model\Tag $tagFactory,
        array $data = []
        ) {
    ...
    {
        parent::_construct();
        $this->setId('storelocator_tags_grid');
        $this->setDefaultSort('tag_id');
        $this->setUseAjax(true);
        if ($this->getTag() && $this->getTag()->getTagId()) {
    ...
    protected function _addColumnFilterToCollection($column)
    {
        ...
        if ($column->getId() == 'in_products') {
            $productIds = $this->_getSelectedTags();
            if (empty($productIds)) {
                $productIds = 0;
            }
            if ($column->getFilter()->getValue()) {
                $this->getCollection()->addFieldToFilter('main_table.tag_id', ['in' => $productIds]);
            } else {
                if ($productIds) {
                    $this->getCollection()->addFieldToFilter('main_table.tag_id', ['nin' => $productIds]);
                }
            }
        } else {
    ...
    {
        $collection = $this->_tagFactory->getCollection();
        ...
        if ($this->isReadonly()) {
            ...
            $collection->addFieldToFilter('main_table.tag_id', ['in' => $productIds]);
        }
        $tag = $this->getTag();
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn(
            ...
            [
                'header' => __('Action'),
                'type' => 'action',
                'renderer'  => 'Lof\StoreLocator\Block\Adminhtml\Tag\Renderer\TagAction',
            ]
            );
        // $this->addColumn(
        //     'position',
    ...
    public function getGridUrl()
    {
        ...
        return $this->_getData(
            ...
            'storelocator/*/tagsGrid/tag_id/'.$tag->getTagId(),
            ['_current' => true]
            );
    }
    ...
    public function getSelectedTags()
    {
        ...
        if ($id) {
            ...
            $model = $objectManager->create('Lof\StoreLocator\Model\StoreLocator');
            $model->load($id);

            $tag = $model->getTag();
            if (!is_array($tag)){
                $modelTag = $objectManager->create('Lof\StoreLocator\Model\Tag');
                $_tagValue = $modelTag->selectNameIN($tag);
            }
        }
        return $_tagValue;
    }
    ...
    public function filterProductPosition($collection, $column)
    {
        ...
        $tag = $this->getTag();
        $condition['tag_id'] = $tag->getTagId();
        $collection->addLinkTagToFilter($column->getIndex(), $condition);
        return $this;
    }
}
