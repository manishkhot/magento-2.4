 ...
 */
namespace Lof\StoreLocator\Controller;
    ...
    public function match(RequestInterface $request)
    {
        ...
        if (!$this->dispatched) {
            ...
            if ($enable && count($urlKeys) == 2 && $route == $urlKeys[0]) {
                $alias = urldecode($urlKeys[1]);
                $storeLocator = $this->_storeLocator->getCollection()
                ->addFieldToFilter("is_active", 1)
                ->addFieldToFilter("name", $alias)
                ->addStoreFilter($store)
                ->getFirstItem();
                if(empty($storeLocator->getData())){
                    $storeLocator = $this->_storeLocator->getCollection()
                                    ->addFieldToFilter("is_active", 1)
                                    ->addFieldToFilter("seo_url", $alias)
                                    ->addStoreFilter($store)
                                    ->getFirstItem();
                }
                if(!empty($storeLocator->getData())){
                    ...
                    ->setParam('id', $storeLocator->getId());
                    $request->setAlias(\Magento\Framework\Url::REWRITE_REQUEST_PATH_ALIAS, $origUrlKey);
                    $request->setDispatched(true);
                    $this->dispatched = true;
                    return $this->actionFactory->create(
